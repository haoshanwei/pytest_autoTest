...
# 数据准备
casepath = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data', 'uc_inner_dbinfo_shop.yml')
test_cases = MakeDdt(casepath).makeData()
# 数据准备


class TestUcenterInnerDbInfoShop():
    '''
    基于 yaml 文件数据的自动化case
    '''
    @pytest.mark.parametrize("method, url, params, data, headers, cookies, proxies, status_code, expectData",
                             test_cases)
    def test_inner_dbinfo_shop(self, method, url, params, data, headers, cookies, proxies, status_code, expectData):
        '''/inner/dbinfo/shop/ 节点下测试用例'''
