...
# 数据准备
casepath = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data', 'uc_inner_dbinfo_address.yml')
...
class TestUcenterInnerDbInfoAddress():
    ...
    @pytest.mark.parametrize("method, url, params, data, headers, cookies, proxies, status_code, expectData",
                             test_cases)
    def test_inner_dbinfo_address(self, method, url, params, data, headers, cookies, proxies, status_code, expectData):
