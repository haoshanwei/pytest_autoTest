...
# 数据准备
casepath = findCase(__file__, 'uc_inner_dbinfo_user.yml', n=2)
test_cases = MakeDdt(casepath).makeData_V2()
# 数据准备


class TestUcenterInnerDbInfoUser():
    '''
    ...
    @pytest.mark.parametrize("method, url, params, data, headers, cookies, proxies, status_code, expectData, name",
                             test_cases)
    def test_inner_dbinfo_user(self, method, url, params, data, headers, cookies, proxies, status_code, expectData, name):
        '''/inner/dbinfo/user/ 节点下测试用例'''
